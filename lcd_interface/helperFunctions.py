...
def showMenuDT(display,hardware):
    ...
    """
    display.clear()
    display.text("-Place YELLOW jumper to DT",0,15)
    ...
    if ((vHI > 2.25) and (vHI < 2.75)
        ...
        display.text(f"vHI = {vHI}",0,120)
        display.text(f"vLO = {vLO}", 0,135)
        display.text("Returning to menu", 0,150)
        loading(display)
        return "DT Pass"
    else:
        display.clear()
...
def showMenuCK(display,hardware):
    ...
    """
    display.clear()
    display.text("-Place GREEN jumper to CK",0,15)
...
def showMenuEN(display,hardware):
    ...
    """
    display.clear()
    display.text("-Place ORANGE jumper to EN",0,15)
...
def runManualENTest(display,hardware):
    ...
    if (reading > 2.25) and (reading < 2.75): #test passed
        ...
        display.text(f"reading = {reading}",0,120)
        display.text("Remove jumper from EM_PU", 0,135)
        display.text("Returning to menu", 0,150)
        loading(display)
        return
        ...
        display.text("Test failed: Check connections", 0,105)
        display.text(f"reading = {reading}",0,120)
        display.text("Returning to menu", 88,150)
        loading(display)
        return 
    ...
    while True:
        ...
        if hardware.isPressed():
            if options[highlight] == "YES":
                ...
                display.text("Place jumper on IREF_N/P", 0, 120)
                display.text("Returning to Menu", 0,135)
                loading(display)
                return
            else: #:NO
                display.clear()
