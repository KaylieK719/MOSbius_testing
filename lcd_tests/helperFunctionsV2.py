...
#UPDATES: 
# Added NMOS/PMOS Current Bias Test
# Added 3-Stage Ring Oscillator Test
...
def threeRODirections():
    display.clear()
    display.text("Testing 3-Stage Ring Oscillator:", 0,15)
    display.text("1.Programming Chip",0,45)
    display.text("2.Place scope probes at STAGE1,2, and 3",0,60)
    display.text("(Pins, 45,50,!!CHECK)", 0,75)
    display.text("3.Confirm for correct waveforms", 0,90)
...
def runThreeROTest():
    ...
    while True:
        ...
        if hardwareLCD.isPressed():
            if options[highlight] == "YES":
                display.clear()
                display.text("Passed 3-Stage Ring Oscillator Test",0, 105)
                display.text("Returning to Menu", 0,120)
                for i in range(3):
                    display.text(".", 140+i*8, 135)
                    sleep(1)
